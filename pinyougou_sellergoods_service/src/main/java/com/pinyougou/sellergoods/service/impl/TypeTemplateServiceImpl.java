...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	public List<Map> findSpecList(Long id) {
		...
		for (Map map : list) {
			...
			List<TbSpecificationOption> options = specMapper.select(where);
			//保存数据到结果列表中
	...
	/**
	 * 将所有的spec_ids规格信息,brand_ids 品牌信息存入redis
	 */
	private void saveToRedis(){
		...
		for (TbTypeTemplate template : templates) {
			...
			//获取品牌信息
			String brandString = template.getBrandIds();
			List<Map> brandList  = JSON.parseArray(brandString, Map.class);
